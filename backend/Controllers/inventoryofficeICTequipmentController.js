// controllers/inventoryOfficeICTEquipmentController.js
...
/**
 * Adds logging (CREATE / UPDATE / DELETE + error cases) for ICT Equipment.
 *
 * Best practice:
 * - If you have JWT/auth middleware, attach the authenticated user to req.user.
 *   actorFromReq() will pick it up (email/username/id).
 * - If you don't, you can still pass an actor using:
 *     headers: { "x-user-email": "someone@dict.gov.ph" }
...
const getNextNumericIdentifier = async (field) => {
  const docs = await InventoryOfficeICTEquipment.find({ [field]: { $ne: null } })
    .select(field)
    .lean();
  let max = 0;
  docs.forEach((doc) => {
...
const ensureDocNumbers = async (item) => {
  ...
  return InventoryOfficeICTEquipment.findByIdAndUpdate(
    item._id,
    { $set: updates },
    { new: true }
  );
...
const rotateSignedFile = async (doc) => {
  ...
  await InventoryOfficeICTEquipment.updateOne(
    { _id: doc._id },
    {
      $set: {
  ...
  return InventoryOfficeICTEquipment.findById(doc._id);
};
...
export const getAllInventory = async (req, res) => {
  try {
    ...
    if (fetchAll || !hasPaging) {
      const inventoryItems = await InventoryOfficeICTEquipment.find(filter);
      return res.status(200).json(inventoryItems);
    }

    const [data, total] = await Promise.all([
      InventoryOfficeICTEquipment.find(filter)
        .sort({ _id: -1 })
        .skip(skip)
        .limit(limit),
    ...
    await safeCreateLog({
      ...
      action: "Office ICT Equipment - List",
      status: "error",
      time: nowISO(),
    });
...
export const createInventoryItem = async (req, res) => {
  try {
    const maxIdItem = await InventoryOfficeICTEquipment.findOne(
      {},
      {},
      { sort: { id: -1 } }
    );
    ...
    let newItem = await InventoryOfficeICTEquipment.create({
      ...req.body,
      id: newId,
      item_no: newItemNo,
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office ICT Equipment - Create (${fmtItemLabel(newItem)})`,
      status: "success",
      time: nowISO(),
    });
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: "Office ICT Equipment - Create",
      status: "error",
      time: nowISO(),
    });
...
export const updateInventoryItem = async (req, res) => {
  ...
  try {
    // fetch BEFORE for better log detail (status/requeststatus diffs)
    const before = await InventoryOfficeICTEquipment.findById(itemId);
    if (!before) {
      await safeCreateLog({
        userId: actorFromReq(req),
        action: `Office ICT Equipment - Update (Not Found: ${itemId})`,
        status: "error",
        time: nowISO(),
      });
    ...
    if (nextStatusLower === "for transfer" && beforeStatusLower !== "for transfer") {
      ...
      // Checkform resolves the recipient from these id aliases first. Sync
      // them with the selected recipient instead of retaining an old holder.
    ...
    if (Object.prototype.hasOwnProperty.call(updateData, "property_no")) {
      ...
      if (propNo !== normalizeLooseText(before.property_no)) {
        const duplicate = await InventoryOfficeICTEquipment.findOne({
          _id: { $ne: itemId },
          $expr: {
    ...
    let updatedItem = await InventoryOfficeICTEquipment.findOneAndUpdate(
    ...
    if (shouldRecalcAsset) {
      ...
      if (
        ...
        updatedItem = await InventoryOfficeICTEquipment.findById(updatedItem._id);
      }
    }
    ...
    let action = `Office ICT Equipment - Update (${fmtItemLabel(updatedItem)})`;

    const changes = buildChangeList(before, updatedItem);

    // Make the log more meaningful for the usual workflow events
    if (statusChanged) {
      const s = afterStatus.toLowerCase().trim();

      if (s === "issued") {
        action = `Office ICT Equipment - Issued (${fmtItemLabel(updatedItem)})`;
      } else if (s === "for return to inventory") {
        action = `Office ICT Equipment - Return to Inventory Requested (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "in stock" && isReturnToInventoryStatus(beforeStatus)) {
        action = `Office ICT Equipment - Returned to Inventory (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "for transfer") {
        action = `Office ICT Equipment - Transfer Requested (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "transferred" || s === "trasferred") {
        action = `Office ICT Equipment - Transferred (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "for disposal") {
        action = `Office ICT Equipment - Disposal Requested (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "disposed") {
        action = `Office ICT Equipment - Disposed (${fmtItemLabel(updatedItem)})`;
      } else {
        action = `Office ICT Equipment - Status Update (${fmtItemLabel(
          updatedItem
        )}): "${beforeStatus}" → "${afterStatus}"`;
      }
    } else if (reqStatusChanged) {
      action = `Office ICT Equipment - Requeststatus Update (${fmtItemLabel(
        updatedItem
      )}): "${beforeReqStatus}" → "${afterReqStatus}"`;
    }

    // If both changed, keep a combined detail line
    if (statusChanged && reqStatusChanged) {
      action = `Office ICT Equipment - Update (${fmtItemLabel(
        updatedItem
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office ICT Equipment - Update (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
export const deleteInventoryItem = async (req, res) => {
  ...
  try {
    const existing = await InventoryOfficeICTEquipment.findById(itemId);
    const deletedItem = await InventoryOfficeICTEquipment.findByIdAndDelete(
      itemId
    );
    ...
    if (!deletedItem) {
      await safeCreateLog({
        userId: actorFromReq(req),
        action: `Office ICT Equipment - Delete (Not Found: ${itemId})`,
        status: "error",
        time: nowISO(),
      });
      return res.status(404).json({ error: "Inventory item not found" });
    ...
    if (existing?.thumbnail?.stored_name) {
      await removeThumbnailFile(existing.thumbnail.stored_name, "officeict");
    }

    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office ICT Equipment - Delete (${fmtItemLabel(
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office ICT Equipment - Delete (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
// Thumbnail upload
export const uploadIctThumbnail = async (req, res) => {
  const itemId = req.params.id;
  ...
  try {
    ...
    const existing = await InventoryOfficeICTEquipment.findById(itemId);
    if (!existing) {
      await removeThumbnailFile(req.file.filename, "officeict");
      await safeCreateLog({
        userId: actor,
        action: `Office ICT Equipment - Thumbnail Upload (Not Found: ${itemId})`,
        status: "error",
        time: nowISO(),
      });
    ...
    existing.thumbnail = {
      url: thumbnailFileUrl(req.file.filename, "officeict"),
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office ICT Equipment - Thumbnail Upload (${fmtItemLabel(existing)})`,
      status: "success",
      time: nowISO(),
    });
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office ICT Equipment - Thumbnail Upload (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
export const uploadSignedInventoryFile = async (req, res) => {
  ...
  try {
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office ICT Equipment - Signed File Uploaded (${fmtItemLabel(existing)})`,
      status: "success",
      time: nowISO(),
    });
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office ICT Equipment - Signed File Upload (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
export const revertSignedInventoryFile = async (req, res) => {
  ...
  try {
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office ICT Equipment - Signed File Reverted (${fmtItemLabel(existing)})`,
      status: "success",
      time: nowISO(),
    });
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office ICT Equipment - Signed File Revert (${itemId})`,
      status: "error",
      time: nowISO(),
    });
    return res
...
export const revertInventoryState = async (req, res) => {
  ...
  try {
    ...
    const updateResult = await InventoryOfficeICTEquipment.updateOne(
    ...
    for (const storedName of signedFilesToRemove) {
      try {
        ...
        console.error("Failed to clean up signed file after ICT state revert:", fileError);
      }
    }

    await safeCreateLog({
      userId: actor,
      action: `Office ICT Equipment - Revert State (${fmtItemLabel(existing)})`,
      status: "success",
      time: nowISO(),
    });

    const updated = await InventoryOfficeICTEquipment.findById(itemId);
    return res.status(200).json({ success: true, data: updated });
  } catch (error) {
    console.error("Error reverting ICT inventory state:", error);
    await safeCreateLog({
      userId: actor,
      action: `Office ICT Equipment - Revert State (${itemId})`,
      status: "error",
      time: nowISO(),
    });
    return res
  ...
  try {
    ...
    if (role === "employee") {
      ...
      const roleFilter = involvedFilter
        ? { $or: [instockFilter, involvedFilter] }
        : instockFilter;
      inventoryItem = await InventoryOfficeICTEquipment.findOne({
        _id: itemId,
        ...roleFilter,
      });
      if (!inventoryItem) {
      ...
      inventoryItem = await InventoryOfficeICTEquipment.findById(itemId);
    }
    if (!inventoryItem) {
      await safeCreateLog({
        userId: actorFromReq(req),
        action: `Office ICT Equipment - View (Not Found: ${itemId})`,
        status: "error",
        time: nowISO(),
      });
      return res.status(404).json({ error: "Inventory item not found" });
    ...
    // Optional: If you want view logs (usually too noisy), uncomment:
    // await safeCreateLog({
    //   userId: actorFromReq(req),
    //   action: `Office ICT Equipment - View (${fmtItemLabel(inventoryItem)})`,
    //   status: "success",
    //   time: nowISO(),
    // });
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office ICT Equipment - View (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
export const getPublicInventoryItem = async (req, res) => {
  const itemId = req.params.id;
  try {
    const inventoryItem = await InventoryOfficeICTEquipment.findById(itemId);
    if (!inventoryItem) {
      return res.status(404).json({ error: "Inventory item not found" });
    }
    await ensurePublicToken(inventoryItem);
