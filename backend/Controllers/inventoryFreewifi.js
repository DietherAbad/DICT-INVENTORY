...
export const getAllInventory = async (req, res) => {
   try {
      ...
      if (!hasPaging) {
         const inventoryItems = await InventoryFreewifi.find(filter);
         return res.status(200).json(inventoryItems);
      }

      const [data, total] = await Promise.all([
         InventoryFreewifi.find(filter)
            .sort({ _id: -1 })
            .skip(skip)
            .limit(limit),
         InventoryFreewifi.countDocuments(filter),
      ]);
...
export const createInventoryItem = async (req, res) => {
   try {
      const maxIdItem = await InventoryFreewifi.findOne({}, {}, { sort: { 'id': -1 } });
      const maxItemNoItem = await InventoryFreewifi.findOne({}, {}, { sort: { 'item_no': -1 } });
      const maxPropertyNoItem = await InventoryFreewifi.findOne({}, {}, { sort: { 'property_no': -1 } });
      ...
      const newItem = await InventoryFreewifi.create({ ...req.body, id: newId, item_no: newItemNo, property_no: newPropertyNo });

      await safeCreateLog({
         userId: actorFromReq(req),
         action: `Freewifi - Create (${newItem?.itemName || newItem?.description || newItem?._id})`,
         status: "success",
         time: nowISO(),
      });
...
export const updateInventoryItem = async (req, res) => {
   ...
   try {
      const before = await InventoryFreewifi.findById(itemId);
      if (!before) {
      ...
      const updatedItem = await InventoryFreewifi.findOneAndUpdate(
      ...
      let action = `Freewifi - Update (${baseLabel})`;
...
export const deleteInventoryItem = async (req, res) => {
   const itemId = req.params.id;
   try {
      const existing = await InventoryFreewifi.findById(itemId);
      const deletedItem = await InventoryFreewifi.findByIdAndDelete(itemId);
      if (!deletedItem) {
         await safeCreateLog({
            userId: actorFromReq(req),
            action: `Freewifi - Delete (Not Found: ${itemId})`,
            status: "error",
            time: nowISO(),
         });
         return res.status(404).json({ error: 'Inventory item not found' });
      ...
      if (existing?.thumbnail?.stored_name) {
         await removeThumbnailFile(existing.thumbnail.stored_name, "freewifi");
      }
      await safeCreateLog({
         userId: actorFromReq(req),
         action: `Freewifi - Delete (${deletedItem?.itemName || deletedItem?.description || deletedItem?._id})`,
         status: "success",
         time: nowISO(),
      });
...
export const uploadFreewifiThumbnail = async (req, res) => {
   const itemId = req.params.id;
   ...
   try {
      ...
      const existing = await InventoryFreewifi.findById(itemId);
      if (!existing) {
         await removeThumbnailFile(req.file.filename, "freewifi");
      ...
      existing.thumbnail = {
         url: thumbnailFileUrl(req.file.filename, "freewifi"),
...
export const getInventoryItem = async (req, res) => {
   const itemId = req.params.id;
   try {
      const inventoryItem = await InventoryFreewifi.findById(itemId);
      if (!inventoryItem) {
         return res.status(404).json({ error: 'Inventory item not found' });
      }
