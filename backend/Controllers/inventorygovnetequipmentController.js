...
export const getAllInventory = async (req, res) => {
   try {
      ...
      if (!hasPaging) {
         const inventoryItems = await InventoryGovnetEquipment.find(filter);
         return res.status(200).json(inventoryItems);
      }

      const [data, total] = await Promise.all([
         InventoryGovnetEquipment.find(filter)
            .sort({ _id: -1 })
            .skip(skip)
            .limit(limit),
         InventoryGovnetEquipment.countDocuments(filter),
      ]);
...
export const createInventoryItem = async (req, res) => {
   try {
      const maxIdItem = await InventoryGovnetEquipment.findOne({}, {}, { sort: { 'id': -1 } });
      const maxItemNoItem = await InventoryGovnetEquipment.findOne({}, {}, { sort: { 'item_no': -1 } });
      const maxPropertyNoItem = await InventoryGovnetEquipment.findOne({}, {}, { sort: { 'property_no': -1 } });
      ...
      const newItem = await InventoryGovnetEquipment.create({ ...req.body, id: newId, item_no: newItemNo, property_no: newPropertyNo });

      await safeCreateLog({
         userId: actorFromReq(req),
         action: `Govnet Equipment - Create (${newItem?.itemName || newItem?.description || newItem?._id})`,
         status: "success",
         time: nowISO(),
      });
     ...
     await safeCreateLog({
        userId: actorFromReq(req),
        action: "Govnet Equipment - Create",
        status: "error",
        time: nowISO(),
     });
...
export const updateInventoryItem = async (req, res) => {
   ...
   try {
      const before = await InventoryGovnetEquipment.findById(itemId);
      if (!before) {
      ...
      const updatedItem = await InventoryGovnetEquipment.findOneAndUpdate(
      ...
      let action = `Govnet Equipment - Update (${baseLabel})`;
      ...
      await safeCreateLog({
         userId: actorFromReq(req),
         action: `Govnet Equipment - Update (${itemId})`,
         status: "error",
         time: nowISO(),
      });
      res.status(400).json({ error: 'Invalid data provided' });
...
export const deleteInventoryItem = async (req, res) => {
   const itemId = req.params.id;
   try {
      const existing = await InventoryGovnetEquipment.findById(itemId);
      const deletedItem = await InventoryGovnetEquipment.findByIdAndDelete(itemId);
      if (!deletedItem) {
         await safeCreateLog({
            userId: actorFromReq(req),
            action: `Govnet Equipment - Delete (Not Found: ${itemId})`,
            status: "error",
            time: nowISO(),
         });
         return res.status(404).json({ error: 'Inventory item not found' });
      ...
      if (existing?.thumbnail?.stored_name) {
         await removeThumbnailFile(existing.thumbnail.stored_name, "govnetequipment");
      }
      await safeCreateLog({
         userId: actorFromReq(req),
         action: `Govnet Equipment - Delete (${deletedItem?.itemName || deletedItem?.description || deletedItem?._id})`,
         status: "success",
         time: nowISO(),
      });
     ...
     await safeCreateLog({
        userId: actorFromReq(req),
        action: `Govnet Equipment - Delete (${itemId})`,
        status: "error",
        time: nowISO(),
     });
     res.status(500).json({ error: 'Internal server error' });
...
export const uploadGovnetEquipmentThumbnail = async (req, res) => {
   const itemId = req.params.id;
   ...
   try {
      ...
      const existing = await InventoryGovnetEquipment.findById(itemId);
      if (!existing) {
         await removeThumbnailFile(req.file.filename, "govnetequipment");
      ...
      existing.thumbnail = {
         url: thumbnailFileUrl(req.file.filename, "govnetequipment"),
...
export const getInventoryItem = async (req, res) => {
   const itemId = req.params.id;
   try {
      const inventoryItem = await InventoryGovnetEquipment.findById(itemId);
      if (!inventoryItem) {
         return res.status(404).json({ error: 'Inventory item not found' });
      }
