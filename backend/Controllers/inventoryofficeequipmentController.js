// controllers/inventoryOfficeEquipmentController.js
...
/**
 * LOGGING NOTES:
 * - This will create Logs entries for CREATE / UPDATE / DELETE (and error cases).
 * - userId is best taken from your auth middleware (req.user.email / req.user.username / req.user._id).
 * - If you don't have req.user yet, you can still send the actor using:
...
const actorFromReq = (req) => {
  // Prefer auth middleware (req.user.*), else fall back to headers/body
  return (
    req?.user?.email ||
    req?.user?.username ||
    req?.user?.id ||
...
const getNextNumericIdentifier = async (field) => {
  const docs = await InventoryOfficeEquipment.find({ [field]: { $ne: null } })
    .select(field)
    .lean();
  let max = 0;
  docs.forEach((doc) => {
...
const fmtItemLabel = (item) => {
  const pn = item?.property_no ?? item?.propertyNo ?? "";
...
const safeCreateLog = async (logData) => {
  try {
    ...
    // Never fail the main endpoint because of logging
...
const ensureDocNumbers = async (item) => {
  ...
  return InventoryOfficeEquipment.findByIdAndUpdate(
    item._id,
    { $set: updates },
    { new: true }
  );
...
const rotateSignedFile = async (doc) => {
  ...
  await InventoryOfficeEquipment.updateOne(
    { _id: doc._id },
    {
      $set: {
  ...
  return InventoryOfficeEquipment.findById(doc._id);
};
...
export const getAllInventory = async (req, res) => {
  try {
    ...
    if (fetchAll || !hasPaging) {
      const inventoryItems = await InventoryOfficeEquipment.find(filter);
      return res.status(200).json(inventoryItems);
    }

    const [data, total] = await Promise.all([
      InventoryOfficeEquipment.find(filter)
        .sort({ _id: -1 })
        .skip(skip)
        .limit(limit),
...
export const createInventoryItem = async (req, res) => {
  try {
    const maxIdItem = await InventoryOfficeEquipment.findOne({}, {}, { sort: { id: -1 } });
    ...
    let newItem = await InventoryOfficeEquipment.create({
      ...req.body,
      id: newId,
      item_no: newItemNo,
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office Equipment - Create (${fmtItemLabel(newItem)})`,
      status: "success",
      time: nowISO(),
    });
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: "Office Equipment - Create",
      status: "error",
      time: nowISO(),
    });
...
// UPDATE (logs status transitions like Issued/For Transfer/For Disposal/Disposed/Transferred)
export const updateInventoryItem = async (req, res) => {
  ...
  try {
    const before = await InventoryOfficeEquipment.findById(itemId);
    if (!before) {
      await safeCreateLog({
        userId: actorFromReq(req),
        action: `Office Equipment - Update (Not Found: ${itemId})`,
        status: "error",
        time: nowISO(),
      });
    ...
    if (nextStatusLower === "for transfer" && beforeStatusLower !== "for transfer") {
      ...
      // Keep all legacy recipient-id aliases in sync. Checkform prefers the
      // *_id values, so retaining an old holder here misclassifies a transfer
      // as a return-to-issued transaction.
    ...
    if (Object.prototype.hasOwnProperty.call(updateData, "property_no")) {
      ...
      if (propNo !== normalizeLooseText(before.property_no)) {
        const duplicate = await InventoryOfficeEquipment.findOne({
          _id: { $ne: itemId },
          $expr: {
    ...
    let updatedItem = await InventoryOfficeEquipment.findOneAndUpdate(
    ...
    if (shouldRecalcAsset) {
      ...
      if (
        ...
        updatedItem = await InventoryOfficeEquipment.findById(updatedItem._id);
      }
    }
    ...
    let action = `Office Equipment - Update (${fmtItemLabel(updatedItem)})`;

    const changes = buildChangeList(before, updatedItem);

    // Prefer “business” action labels for common flows
    if (statusChanged) {
      const s = afterStatus.toLowerCase().trim();
      if (s === "issued") {
        action = `Office Equipment - Issued (${fmtItemLabel(updatedItem)})`;
      } else if (s === "for return to inventory") {
        action = `Office Equipment - Return to Inventory Requested (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "in stock" && isReturnToInventoryStatus(beforeStatus)) {
        action = `Office Equipment - Returned to Inventory (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "for transfer") {
        action = `Office Equipment - Transfer Requested (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "transferred") {
        action = `Office Equipment - Transferred (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "for disposal") {
        action = `Office Equipment - Disposal Requested (${fmtItemLabel(
          updatedItem
        )})`;
      } else if (s === "disposed") {
        action = `Office Equipment - Disposed (${fmtItemLabel(updatedItem)})`;
      } else {
        action = `Office Equipment - Status Update (${fmtItemLabel(
          updatedItem
        )}): "${beforeStatus}" → "${afterStatus}"`;
      }
    } else if (reqStatusChanged) {
      action = `Office Equipment - Requeststatus Update (${fmtItemLabel(
        updatedItem
      )}): "${beforeReqStatus}" → "${afterReqStatus}"`;
    }

    // If both changed and we didn’t hit a specific status label, keep detailed info
    if (
      statusChanged &&
      reqStatusChanged &&
      !/Requested|Issued|Transferred|Disposed|Status Update/.test(action)
    ) {
      action = `Office Equipment - Update (${fmtItemLabel(
        updatedItem
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office Equipment - Update (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
export const deleteInventoryItem = async (req, res) => {
  ...
  try {
    const existing = await InventoryOfficeEquipment.findById(itemId);
    const deletedItem = await InventoryOfficeEquipment.findByIdAndDelete(itemId);

    if (!deletedItem) {
      await safeCreateLog({
        userId: actorFromReq(req),
        action: `Office Equipment - Delete (Not Found: ${itemId})`,
        status: "error",
        time: nowISO(),
      });
      return res.status(404).json({ error: "Inventory item not found" });
    ...
    if (existing?.thumbnail?.stored_name) {
      await removeThumbnailFile(existing.thumbnail.stored_name, "officeequipment");
    }

    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office Equipment - Delete (${fmtItemLabel(existing || deletedItem)})`,
      status: "success",
      time: nowISO(),
    });
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office Equipment - Delete (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
// Thumbnail upload
export const uploadEquipmentThumbnail = async (req, res) => {
  const itemId = req.params.id;
  ...
  try {
    ...
    const existing = await InventoryOfficeEquipment.findById(itemId);
    if (!existing) {
      await removeThumbnailFile(req.file.filename, "officeequipment");
      await safeCreateLog({
        userId: actor,
        action: `Office Equipment - Thumbnail Upload (Not Found: ${itemId})`,
        status: "error",
        time: nowISO(),
      });
    ...
    existing.thumbnail = {
      url: thumbnailFileUrl(req.file.filename, "officeequipment"),
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office Equipment - Thumbnail Upload (${fmtItemLabel(existing)})`,
      status: "success",
      time: nowISO(),
    });
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office Equipment - Thumbnail Upload (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
export const uploadSignedInventoryFile = async (req, res) => {
  ...
  try {
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office Equipment - Signed File Uploaded (${fmtItemLabel(existing)})`,
      status: "success",
      time: nowISO(),
    });
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office Equipment - Signed File Upload (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
export const revertSignedInventoryFile = async (req, res) => {
  ...
  try {
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office Equipment - Signed File Reverted (${fmtItemLabel(existing)})`,
      status: "success",
      time: nowISO(),
    });
    ...
    await safeCreateLog({
      userId: actor,
      action: `Office Equipment - Signed File Revert (${itemId})`,
      status: "error",
      time: nowISO(),
    });
    return res
...
export const revertInventoryState = async (req, res) => {
  ...
  try {
    ...
    const updateResult = await InventoryOfficeEquipment.updateOne(
    ...
    for (const storedName of signedFilesToRemove) {
      try {
        ...
        console.error("Failed to clean up signed file after state revert:", fileError);
      }
    }

    await safeCreateLog({
      userId: actor,
      action: `Office Equipment - Revert State (${fmtItemLabel(existing)})`,
      status: "success",
      time: nowISO(),
    });

    const updated = await InventoryOfficeEquipment.findById(itemId);
    return res.status(200).json({ success: true, data: updated });
  } catch (error) {
    console.error("Error reverting inventory state:", error);
    await safeCreateLog({
      userId: actor,
      action: `Office Equipment - Revert State (${itemId})`,
      status: "error",
      time: nowISO(),
    });
    return res
  ...
  try {
    ...
    if (role === "employee") {
      ...
      const roleFilter = involvedFilter
        ? { $or: [instockFilter, involvedFilter] }
        : instockFilter;
      inventoryItem = await InventoryOfficeEquipment.findOne({
        _id: itemId,
        ...roleFilter,
      });
      if (!inventoryItem) {
      ...
      inventoryItem = await InventoryOfficeEquipment.findById(itemId);
    }
    if (!inventoryItem) {
      await safeCreateLog({
        userId: actorFromReq(req),
        action: `Office Equipment - View (Not Found: ${itemId})`,
        status: "error",
        time: nowISO(),
      });
      return res.status(404).json({ error: "Inventory item not found" });
    ...
    // Optional: do NOT log every view (too noisy). If you want, uncomment:
    // await safeCreateLog({
    //   userId: actorFromReq(req),
    //   action: `Office Equipment - View (${fmtItemLabel(inventoryItem)})`,
    //   status: "success",
    //   time: nowISO(),
    // });
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: `Office Equipment - View (${itemId})`,
      status: "error",
      time: nowISO(),
    });
...
export const getPublicInventoryItem = async (req, res) => {
  const itemId = req.params.id;
  try {
    const inventoryItem = await InventoryOfficeEquipment.findById(itemId);
    if (!inventoryItem) {
      return res.status(404).json({ error: "Inventory item not found" });
    }
    await ensurePublicToken(inventoryItem);
...
// LIST ISSUED
export const getIssuedInventoryItems = async (req, res) => {
  try {
    const issuedInventoryItems = await InventoryOfficeEquipment.find({
      status: "Issued",
    });
    res.status(200).json(issuedInventoryItems);
  } catch (error) {
    console.error("Error fetching issued inventory items:", error);

    await safeCreateLog({
      userId: actorFromReq(req),
      action: "Office Equipment - List Issued",
      status: "error",
      time: nowISO(),
    });
...
// LIST INSTOCK
export const getInstockInventoryItems = async (req, res) => {
  try {
    const instockInventoryItems = await InventoryOfficeEquipment.find({
      status: "Instock",
    });
    res.status(200).json(instockInventoryItems);
  } catch (error) {
    console.error("Error fetching instock inventory items:", error);

    await safeCreateLog({
      userId: actorFromReq(req),
      action: "Office Equipment - List Instock",
      status: "error",
      time: nowISO(),
    });
...
// OPTIONAL: LIST FOR DISPOSAL
export const getForDisposalInventoryItems = async (req, res) => {
  try {
    const items = await InventoryOfficeEquipment.find({
      status: "For Disposal",
    });
    res.status(200).json(items);
  } catch (error) {
    console.error("Error fetching For Disposal items:", error);

    await safeCreateLog({
      userId: actorFromReq(req),
      action: "Office Equipment - List For Disposal",
      status: "error",
      time: nowISO(),
    });
...
// OPTIONAL: LIST DISPOSED
export const getDisposedInventoryItems = async (req, res) => {
  try {
    const items = await InventoryOfficeEquipment.find({
      status: "Disposed",
    });
    ...
    await safeCreateLog({
      userId: actorFromReq(req),
      action: "Office Equipment - List Disposed",
      status: "error",
      time: nowISO(),
    });
