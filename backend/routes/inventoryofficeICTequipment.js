...
import {
  ...
  uploadIctThumbnail,
} from "../Controllers/inventoryofficeICTequipmentController.js";
...
router.get('/inventory', requireAccess("stocks.office_ict"), getAllInventory);
router.get('/inventory/:id', requireAccess("details.office_ict"), getInventoryItemById);
...
router.put(
  '/inventory/:id',
  requireAccess("details.office_ict"),
  writeLimiter,
  validateBody(schemas.inventoryPayload),
  updateInventoryItem
);
...
router.post(
  ...
  requireAccess("stocks.entry"),
  writeLimiter,
  createThumbnailUpload("officeict").single("thumbnail"),
  uploadIctThumbnail
);
router.post(
