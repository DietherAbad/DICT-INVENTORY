...
export function PurchaseformICTequipment() {
  ...
  useEffect(() => {
    const fetchClassificationOptions = async () => {
      try {
        ...
        const classifications = data.filter(item => item.designation === "ICT Equipments");
  ...
  useEffect(() => {
    const fetchUnitMeasures = async () => {
      try {
        ...
        const data = await res.json();
        const filteredData = data.filter(item => item.designation === "ICT Equipments");
    ...
    const fetchIssuedToOptions = async () => {
      try {
        ...
        const filteredData = data.filter(user =>
          ["Regional Director", "TOD", "AFD", "Isabela Provincial Officer", "Cagayan Provincial Officer", "Nueva Vizcaya Provincial Officer", "Batanes Provincial Officer", "Quirino Provincial Officer"].includes(user.role)
        );
        const usernames = filteredData.map(user => user.username);
        setIssuedToOptions(usernames);
      } catch (error) {
  ...
  const handleStockinAndSubmission = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/inventoryofficeICTequipment/inventory`, {
        method: 'post',
        headers: {
  ...
  return (
    <>
      ...
      {showSuccessPopup && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 shadow-lg">
            <p className="text-lg font-semibold mb-4">Successfully submitted.</p>
            <button
              onClick={() => {
      ...
      {showErrorPopup && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 shadow-lg">
            <p className="text-lg font-semibold mb-4">An error occurred while processing your request.</p>
            <button
      ...
      {showFieldErrorPopup && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 shadow-lg">
            <p className="text-lg font-semibold mb-4">Please fill all required fields.</p>
            <button
