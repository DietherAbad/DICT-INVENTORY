...
export function Purchaseformfurnitureandfixture({
  ...
  // stored_to UI (same pattern as equipment form)
  ...
  const [successMessage, setSuccessMessage] = useState(
    "Your purchase details have been successfully submitted."
  );
  ...
  const { clear: clearDraft } = useFormDraft(
    "draft:purchase:office-furniture",
    draftPayload,
    (draft) => {
      if (draft?.stock) {
  ...
  const { suggestion: smartSuggestion, score: smartScore } = useSmartItemPrefill({
    endpoint: `${BASE_URL}/inventoryofficefurnitureandfixture/inventory`,
    ...
    const fetchClassificationOptions = async () => {
      try {
        ...
        const classifications = data
          .filter(
            (item) =>
              item.designation === "Furniture & Fixture" ||
              item.designation === "Furniture & Fixtures"
          )
  ...
  useEffect(() => {
    const fetchIssuedToOptions = async () => {
      try {
        ...
        // Map EVERYONE; keep position for display (same behavior as equipment form)
        setIssuedToOptions(
          data.map((u) => ({
            _id: u._id,
            username: u.username,
  ...
  // recalc total_cost with numeric values
  useEffect(() => {
    setStock((prev) => {
  ...
  const uploadThumbnail = async (itemId, file) => {
    ...
    const res = await fetch(
      `${BASE_URL}/inventoryofficefurnitureandfixture/inventory/${itemId}/thumbnail`,
      {
        method: "POST",
        credentials: "include",
        body: formData,
  ...
  const handleStockinAndSubmission = async (e) => {
    ...
    try {
      ...
      setSuccessMessage("Your purchase details have been successfully submitted.");
      setThumbnailError("");
      ...
      if (result && typeof result === "object") {
        let finalMessage = "Your purchase details have been successfully submitted.";
        if (thumbnailFile) {
          try {
  ...
  const applyFurniturePrefill = (item, { applyName = false } = {}) => {
    if (!item) return;
    setStock((prev) => {
      const next = { ...prev };
  ...
  useEffect(() => {
    ...
    if (lastPrefillIdRef.current === (smartSuggestion._id || smartSuggestion.itemName))
      return;
    applyFurniturePrefill(smartSuggestion);
    ...
    >
          ...
          <div className="w-full">
            <div className="px-2">
              <div className="bg-white/95 rounded-2xl shadow-[0_20px_60px_rgba(15,23,42,0.08)] border border-slate-200/80 mt-6 pb-8 backdrop-blur">
                {/* Header (kept furniture-specific, but layout same as equipment) */}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-900">
                        Purchase Office Furniture & Fixtures
                      </p>
                      <p className="text-xs text-slate-500">
                        Record newly acquired furniture with issuance details.
                      </p>
                    </div>
                  </div>
                </div>

                {/* Purchase Details (same layout as equipment form) */}
                <div className="mt-6 px-7">
                  ...
                  <div className="grid w-full grid-cols-1 lg:grid-cols-2 gap-7 mt-6">
                    ...
                    {/* Total cost (read-only, same behavior as equipment improvement) */}
                    <div>
                      <p className="text-[12px] font-medium tracking-[0.08em] text-slate-600">
                        Total cost (auto)
                      </p>
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-[11px] text-slate-400">
                        Optional. If set, this item will require{" "}
                ...
                {/* Stored to (same behavior as improved equipment form: dropdown + custom toggle) */}
                ...
                <div className="pt-6 px-7">
                  ...
                  <input
                    placeholder="Full description (ex: Executive Office Table, 1.6m, mahogany finish)"
                    className="w-full mt-3 rounded-2xl border border-slate-200/80 bg-slate-50/80 px-5 py-4 text-base text-slate-700 placeholder:text-slate-300 focus:border-blue-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100"
                    ...
                    <div className="mt-2 rounded-xl border border-indigo-100 bg-indigo-50 px-3 py-2 text-[11px] text-indigo-700">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            applyFurniturePrefill(smartSuggestion, { applyName: true })
                          }
                        >
                          Use suggestion
                        </button>
                  ...
                  <p className="mt-1 text-[11px] text-slate-400">
                    Use a precise description for tracking and reporting.
                  </p>
                </div>
                ...
                <div className="pt-4 px-7">
                  ...
                  <input
                    placeholder="Chairs, cushions, add-ons, etc."
                    className="w-full mt-3 rounded-2xl border border-slate-200/80 bg-slate-50/80 px-5 py-4 text-base text-slate-700 placeholder:text-slate-300 focus:border-blue-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100"
                ...
                <div className="pt-4 px-7">
                  ...
                  <input
                    placeholder="PO #, Supplier, location notes, etc."
                    className="w-full mt-3 rounded-2xl border border-slate-200/80 bg-slate-50/80 px-5 py-4 text-base text-slate-700 placeholder:text-slate-300 focus:border-blue-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100"
                    value={stock.remarks}
                    onChange={(e) =>
                ...
                {/* Project (placed at bottom same as equipment layout) */}
                <div className="pt-4 px-7">
                  <p className="text-[12px] font-medium tracking-[0.08em] text-slate-600">
                  ...
                  <p className="mt-2 text-[11px] text-slate-400">
                    Tag the item to the appropriate project.
                  </p>
                </div>
      ...
      >
        <div className="space-y-4 text-center">
          ...
          <p className="text-sm text-slate-600">
            An error occurred while submitting your purchase details. Please try again.
          </p>
          <button
