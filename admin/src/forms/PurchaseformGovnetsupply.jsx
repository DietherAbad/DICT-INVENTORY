...
export function PurchaseformGovnetsupply() {
 ...
 useEffect(() => {
  setstock(prevstock => ({
      ...
      purchase_total_cost: prevstock.stock_total_cost,
...
const classificationOptions = ['Printer Ink', 'Tape', 'Glue'];
...
const uploadThumbnail = async (itemId, file) => {
  ...
  const res = await fetch(
    `${BASE_URL}/inventorygovnetsupply/inventory/${itemId}/thumbnail`,
    {
      method: 'POST',
      credentials: 'include',
      body: formData,
...
const handleStockinAndSubmission = async (e) => {
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}/inventorygovnetsupply/inventory`, {
      method: 'post',
      headers: {
