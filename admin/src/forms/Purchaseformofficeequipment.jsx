...
export function Purchaseformofficeequipment({
  ...
  const [issuedToOptions, setIssuedToOptions] = useState([]); // [{ _id, username, email, position }]
  ...
  // optional allow-list (if you want to restrict to specific IDs later)
  const issuedToIds = [];
  ...
  const { clear: clearDraft } = useFormDraft(
    "draft:purchase:office-equipment",
    draftPayload,
    (draft) => {
      if (draft?.stock) {
  ...
  const { suggestion: smartSuggestion, score: smartScore } = useSmartItemPrefill({
    endpoint: `${BASE_URL}/inventoryofficeequipment/inventory`,
    query: stock.itemName,
  });

  /* ------------------------ effects: fetch options ------------------------ */
  useEffect(() => {
    const fetchClassificationOptions = async () => {
      try {
        ...
        const classifications = data
          .filter((item) => item.designation === "Office Equipments")
  ...
  // "Issued to" can map everyone; we keep position for display
  useEffect(() => {
    const fetchIssuedToOptions = async () => {
      try {
        ...
        // Optional allow list by _id (if you decide to use it)
        if (issuedToIds.length > 0) {
          const allow = new Set(issuedToIds);
          filteredUsers = data.filter((u) => allow.has(u._id));
        }
  ...
  // focus classification search input
  useEffect(() => {
  ...
  // recalc total_cost on qty/unit_cost change
  useEffect(() => {
    setStock((prev) => {
  ...
  const uploadThumbnail = async (itemId, file) => {
    ...
    const res = await fetch(
      `${BASE_URL}/inventoryofficeequipment/inventory/${itemId}/thumbnail`,
      {
        method: "POST",
        credentials: "include",
        body: formData,
  ...
  /* ------------------------ derived display ------------------------ */
  const selectedIssuedTo = issuedToOptions.find(
    (u) => u._id === stock.issued_to
  );

  const selectedIssuedToLabel = selectedIssuedTo
    ? `${selectedIssuedTo.username}${
        selectedIssuedTo.position
          ? ` — ${selectedIssuedTo.position}`
          : ""
      }`
    : "";
  ...
  const applyEquipmentPrefill = (item, { applyName = false } = {}) => {
    if (!item) return;
    setStock((prev) => {
      const next = { ...prev };
  ...
  useEffect(() => {
    ...
    if (lastPrefillIdRef.current === (smartSuggestion._id || smartSuggestion.itemName))
      return;
    applyEquipmentPrefill(smartSuggestion);
    ...
    >
            ...
            <div className="px-2">
              <div className="bg-white/95 rounded-2xl shadow-[0_20px_60px_rgba(15,23,42,0.08)] border border-slate-200/80 mt-6 pb-8 backdrop-blur">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Record newly acquired equipment and optional issuance
                        details.
                      </p>
                    </div>
                  </div>
                ...
                <div className="mt-6 px-7">
                  ...
                  <div className="grid w-full grid-cols-1 lg:grid-cols-2 gap-7 mt-6">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-[11px] text-slate-400">
                        Optional. If set, item will require{" "}
                ...
                <div className="pt-6 mt-2 px-7">
                  ...
                  <input
                    placeholder="Full product description/model..."
                    className="w-full mt-3 rounded-2xl border border-slate-200/80 bg-slate-50/80 px-5 py-4 text-base text-slate-700 placeholder:text-slate-300 focus:border-blue-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100"
                    ...
                    <div className="mt-2 rounded-xl border border-indigo-100 bg-indigo-50 px-3 py-2 text-[11px] text-indigo-700">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            applyEquipmentPrefill(smartSuggestion, { applyName: true })
                          }
                        >
                          Use suggestion
                        </button>
                  ...
                  <p className="mt-1 text-[11px] text-slate-400">
                    Use a clear, complete description for accurate tracking.
                  </p>
                </div>
                ...
                <div className="pt-4 px-7">
                  ...
                  <input
                    placeholder="PO #, Supplier, Warranty info, etc."
                    className="w-full mt-3 rounded-2xl border border-slate-200/80 bg-slate-50/80 px-5 py-4 text-base text-slate-700 placeholder:text-slate-300 focus:border-blue-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-100"
                    value={stock.remarks}
                    onChange={(e) =>
                ...
                <div className="pt-4 px-7">
                  ...
                  <p className="mt-2 text-[11px] text-slate-400">
                    If not found, add this project in Settings → Projects.
                  </p>
                </div>
      ...
      >
        <div className="space-y-4 text-center">
          ...
          <p className="text-sm text-slate-600">
            An error occurred while submitting your purchase details. Please try
            again.
          </p>
          <button
