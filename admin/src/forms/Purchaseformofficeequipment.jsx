...
export function Purchaseformofficeequipment() {
  ...
  useEffect(() => {
    const fetchClassificationOptions = async () => {
      try {
        ...
        const classifications = data.filter(item => item.designation === "Office Equipments");
        setClassificationOptions(classifications.map(item => item.description));
      } catch (error) {
  ...
  useEffect(() => {
    const fetchUnitMeasures = async () => {
      try {
        ...
        const data = await res.json();
        const filteredData = data.filter(item => item.designation === "Office Equipments");
  ...
  useEffect(() => {
    const fetchIssuedToOptions = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error('Failed to fetch issued to options');
        }
        const data = await res.json();
        const roles = [
          'Regional Director', 'TOD', 'AFD', 'Isabela Provincial Officer',
          'Cagayan Provincial Officer', 'Nueva Vizcaya Provincial Officer',
          'Batanes Provincial Officer', 'Quirino Provincial Officer'
        ];
        const filteredUsers = data.filter(user => roles.includes(user.role));
        
        // Fix: Set the issuedToOptions state
        const options = filteredUsers.map(user => user.username);
        setIssuedToOptions(options); // <-- Add this line
      } catch (error) {
  ...
  function showDropDownMenuOne_form_layout_wizard4() {
  ...
  function showDropDownMenuOne_form_layout_wizard5() {
  ...
  const [stock, setStock] = useState({
    ...
    unit_cost: 0,
    itemName: '',
    date_acquired: '', // Initialize date as an empty string
    status: 'Issued',
    total_cost: 0,
    remarks: '',
    archive: false,
...
// If issued_to is not null or blank, set date_issued to the current date
if (stock.issued_to && stock.issued_to.trim() !== '') {
    setStock(prevStock => ({
        ...prevStock,
        date_issued: new Date().toISOString(), // Get current date and time in ISO format
    }));
}
  ...
  const projectOptions = ['General', 'eLGU', 'FreeWifi', 'IIDB', 'ILCDB', 'Tech4Ed'];
  ...
  const handleStockinAndSubmission = async (e) => {
    ...
    if (!stock.classification || !stock.unitofmeasure || !stock.itemName || !stock.qty || !stock.unit_cost || !stock.project || !stock.date_acquired) {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/inventoryofficeequipment/inventory`, {
        method: 'post',
        headers: {
  ...
  const storedToOptions = [
    'TOD', 'AFD', 'RD Office', 'Cagayan PO', 
    'Isabela PO (Cauayan)', 'Isabela PO (Santiago)', 
    'Nueva Vizcaya PO', 'Regional Office'
  ];

  const showDropDownMenuTwo_form_layout_wizard5 = () => {
    setIsStoredToDropdownOpen(!isStoredToDropdownOpen);
  };
  return (
    <>
      <div>
                      ...
                      <div className="relative top-1">
                        <div className="relative w-full mt-2 border border-gray-300 rounded outline-none">
                          ...
                          >
                            ...
                            />
                            {filterOptions(classificationOptions).map((option, index) => (
                    ...
                    <div>
                      ...
                      <div className="relative top-1">
                        <div className="relative w-full mt-2 border border-gray-300 rounded outline-none dropdown-one">
                          ...
                          <div
                            className={`absolute right-0 z-20 ${isIssuedToDropdownOpen ? '' : 'hidden'} w-full px-1 py-2 bg-white border-t border-gray-200 rounded shadow top-12`}
                            id="drop-down-div-one_form_layout_wizard5"
                          >
                            {issuedToOptions.map((option, index) => (
                              <a key={index} href="javascript:void(0)" className="hover">
                                <p
                                  className="p-3 text-sm leading-none text-gray-600 cursor-pointer hover:bg-indigo-100 hover:font-medium hover:text-indigo-700 hover:rounded"
                                  onClick={() => {
                                    handleInputChange('issued_to', option);
                      ...
                      <p className="mt-3 text-xs leading-[15px] text-gray-600">
                        If not found, contact your administrator.
                      </p>
                    </div>
                  </div>
                </div>
                <p className="text-base font-medium leading-none text-gray-800 mt-4 px-7">
    Stored to
  </p>
  <div className="relative top-1 px-7">
    <div className="relative w-full mt-2 border border-gray-300 rounded outline-none dropdown-two">
      <button
        onClick={showDropDownMenuTwo_form_layout_wizard5}
        className="relative flex items-center justify-between w-full px-5 py-4"
      >
        <span
          className="pr-4 text-sm font-medium text-gray-600"
      ...
      <div
        className={`absolute right-0 z-20 ${isStoredToDropdownOpen ? '' : 'hidden'} w-full px-1 py-2 bg-white border-t border-gray-200 rounded shadow top-12`}
        id="drop-down-div-two_form_layout_wizard5"
      >
        {storedToOptions.map((option, index) => (
          <a key={index} href="javascript:void(0)" className="hover">
            <p
              onClick={() => {
                handleInputChange('stored_to', option);
                showDropDownMenuTwo_form_layout_wizard5();
              }}
              className="p-3 text-sm leading-none text-gray-600 cursor-pointer hover:bg-indigo-100 hover:font-medium hover:text-indigo-600"
            >
              {option}
            </p>
          </a>
                ...
                {/* ---------- NEW Inclusions (optional) ---------- */}
      <div className="pt-6 border-gray-300 mt-2 px-7">
        <p className="text-base font-semibold leading-4 text-gray-800">
          Inclusions <span className="text-gray-500">(optional)</span>
        </p>
        <input
          placeholder="Accessories, cables, manuals, etc."
          className="w-full p-3 mt-4 border border-gray-300 rounded outline-none focus:bg-gray-50"
        ...
        />
                ...
                <div className="pt-6 border-gray-300 mt-2 px-7">
                  ...
                  <div className="border-gray-300 rounded"></div>
                  <div className="relative top-1">
                    <div className="relative w-full mt-2 border border-gray-300 rounded outline-none dropdown-one">
                      <button
                      ...
                      <div
                        className={`absolute right-0 z-20 ${isProjectDropdownOpen ? '' : 'hidden'} w-full px-1 py-2 bg-white border-t border-gray-200 rounded shadow top-12`}
                  ...
                  <p className="mt-3 text-xs leading-[15px] text-gray-600">
                    If not found, add a project.
                  </p>
                </div>
                <hr className="h-[1px] bg-gray-100 my-4" />
                <div className="flex flex-col flex-wrap items-center justify-center w-full px-7 lg:flex-row lg:justify-end md:justify-end gap-x-4 gap-y-4">
                  <button
                    className={`bg-indigo-700 rounded hover:bg-indigo-600 transform duration-300 ease-in-out text-sm font-medium px-6 py-4 text-white lg:max-w-[144px] w-full ${isButtonClicked ? ' disabled' : ''}`}
      ...
      {showSuccessPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h2 className="text-lg font-semibold mb-4">Submission Successful</h2>
            <p className="mb-4">Your purchase details have been successfully submitted.</p>
            <button
              onClick={() => {
      ...
      {showErrorPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h2 className="text-lg font-semibold mb-4">Submission Failed</h2>
            <p className="mb-4">An error occurred while submitting your purchase details. Please try again.</p>
            <button
      ...
      {showFieldErrorPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h2 className="text-lg font-semibold mb-4">Incomplete Fields</h2>
            <p className="mb-4">Please fill all required fields.</p>
            <button
              onClick={() => setShowFieldErrorPopup(false)}
              className="bg-yellow-500 px-4 py-2 rounded-md text-white hover:bg-yellow-600 focus:outline-none focus:ring focus:ring-yellow-200"
            >
              Close
            </button>
          </div>
