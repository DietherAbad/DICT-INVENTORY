...
function TableRow({ data }) {
  ...
  return (
    <tr>
      <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-center">{data.property_no}</td>
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-center">{data.qty}</td>
      <td className="px-6 py-4 whitespace-nowrap text-sm text-center">{data.unitofmeasure}</td>
      <td className="px-6 py-4 whitespace-nowrap text-sm text-center">{data.unit_cost}</td>
      <td className="px-6 py-4 whitespace-nowrap text-sm text-center">{data.total_cost}</td>
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
        <Link to={`/checkitemgovnetequip/${data._id}`} className="bg-indigo-100 px-3 py-2 rounded text-indigo-600 hover:text-indigo-800">
        Manage
        </Link>
      </td>
    </tr>
...
function Stocktablegovnetequipments() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/inventorygovnetequipment/inventory`)
      .then(response => {
        if (!response.ok) {
  ...
  return (
    <div className="w-full sm:px-6 pt-10">
      <div className="flex items-center mb-4">
        ...
        <p className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800">STOCK TABLE FOR GOVNET EQUIPMENTS</p>
        <input
          type="text"
          placeholder="Search..."
          value={searchQuery}
...
export default Stocktablegovnetequipments;
