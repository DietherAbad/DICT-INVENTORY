  ...
  return (
    <tr>
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-left">
        <Link to={`/checkitemgovnetequip/${data._id}`} className="manage-button">
        Manage
        </Link>
      </td>
    </tr>
    ...
    const load = async () => {
      try {
        ...
        const response = await fetch(
          `${BASE_URL}/inventorygovnetequipment/inventory?${params.toString()}`
        );
        if (!response.ok) {
    ...
    <div className="w-full sm:px-6 pt-10">
      ...
      <div className="flex flex-col gap-3 mb-4">
        <p className="text-base sm:text-lg md:text-xl lg:text-2xl font-semibold text-slate-900">
          Stock Table for GovNet Equipments
        </p>
