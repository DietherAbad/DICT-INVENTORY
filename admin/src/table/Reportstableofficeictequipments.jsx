// src/components/Reportstableofficeictequipments.jsx
...
/** Heuristic: MongoDB ObjectId-like? */
...
/* ========================================================================================
 * Date helpers (for showing Issued/Transferred dates under names)
 * ====================================================================================== */

/** Convert unknown date-like value to a valid Date, else null. */
...
/** Pick the first valid date from a list of candidate keys in the item. */
...
/**
 * Find the latest matching history date by keyword(s).
 * - Accepts history items like: { name, from, to, date, createdAt, updatedAt }
 * - Uses `to` then `from` then `date` then created/updated fields.
 */
function findLatestHistoryDate(history, keywords = []) {
  ...
  for (const h of history) {
    const name = String(h?.name || h?.action || h?.status || "").toLowerCase();
    if (ks.length && !ks.some((k) => name.includes(k))) continue;

    const candidate =
      toValidDate(h?.to) ||
      toValidDate(h?.from) ||
      toValidDate(h?.date) ||
      toValidDate(h?.createdAt) ||
      toValidDate(h?.created_at) ||
      toValidDate(h?.updatedAt) ||
      toValidDate(h?.updated_at);
...
function getStatusMeta(status) {
  ...
  // Normalize common variants between ICT vs others
...
function resolveCurrentHolderDisplay(item, userMap) {
  ...
  const direct =
    ...
    resolveIdOrName(item?.transfered_to_is, userMap);

  if (direct) return direct;

  if (
    norm(item?.transfer_type || item?.transfertype || item?.transferType || item?.transfer_mode) ===
    "other agency"
  ) {
    const agency =
      item?.transfer_target ||
  ...
  return resolveIdOrName(item?.issued_to, userMap) || item?.issued_to_name || "";
}
...
function resolveCurrentLocationDisplay(item, userMap) {
  const storedLocation =
    String(
      item?.stored_to ||
        resolveIdOrName(item?.stored_to_id, userMap) ||
        resolveIdOrName(item?.stored_to_is, userMap) ||
        ""
    ).trim();
...
/* ========================================================================================
 * Timeline resolvers (for the dates displayed under Issued To / Current Holder)
 * ====================================================================================== */

function getIssuedToTimeline(item) {
  const { label: statusLabel } = getStatusMeta(item?.status || "");
  const lowerStatus = norm(statusLabel);
  const lowerType = norm(item?.transfer_type);

  if (lowerStatus === "for disposal" || lowerStatus === "disposed") return null;

  // If transfer is to other agency, show transfer timing
  if (lowerType === "other agency") {
    const dt =
    ...
    if (lowerStatus === "for transfer") return { label: "Transfer requested", date: dt };
    if (lowerStatus === "transferred") return { label: "Transferred", date: dt };
    return { label: "Updated", date: dt };
  }

  // Otherwise, issuance timing (if any)
  const issuedDt =
    pickFirstDateFromKeys(item, [
      "date_issued",
      "issued_at",
  ...
  // If still in stock, do not show issued date
  if (lowerStatus === "in stock") return null;
...
function getCurrentHolderTimeline(item) {
  ...
  if (lowerStatus === "for transfer") {
    const dt =
      pickFirstDateFromKeys(item, [
        "date_requested",
        "requested_at",
        "requestedAt",
        "date_transfer_request",
        "transfer_request_at",
        "updatedAt",
        "updated_at",
...
function TableRow({ data, userMap }) {
  ...
  // Dates for the two columns (shown under the name)
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors duration-150">
      ...
      {/* Issued To (name + date underneath) */}
      <td className="px-4 py-3 whitespace-nowrap text-[10px] md:text-xs font-medium text-left border-b text-gray-800">
        {issuedTo || "—"}
        {issuedTo && issuedTimeline?.date ? (
      ...
      {/* Current Holder (name + date underneath) */}
      <td className="px-4 py-3 whitespace-nowrap text-[10px] md:text-xs font-medium text-left border-b text-gray-800">
        {currentHolder || "—"}
        {currentHolder && holderTimeline?.date ? (
          <div className="mt-0.5 text-[9px] md:text-[10px] font-medium text-gray-500">
...
function Reportstableofficeictequipments({
  ...
  const STORAGE_KEY = `r02.officeICTReports${storageSuffix}.v2`;
  const STORAGE_COLUMNS_KEY = `r02.officeICTReports${storageSuffix}.columns.v2`;
  ...
  const userMap = useMemo(() => {
    ...
    for (const u of users || []) {
      if (u && u._id) {
        map[u._id] = u.username || u.fullname || u.name || u.email || u.displayName || u._id;
      }
    }
    return map;
  }, [users]);
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      try {
        ...
        if (!invRes.ok) throw new Error("Failed to fetch ICT equipment data.");
        const inv = await invRes.json();

        const usr = usersRes.ok ? await usersRes.json() : [];

        if (Array.isArray(inv)) {
          setInventory([...inv].reverse());
          setTotalCount(inv.length);
        } else {
          setInventory(Array.isArray(inv?.data) ? inv.data : []);
          setTotalCount(Number(inv?.total || 0));
        }
  ...
  const sortedItems = useMemo(() => {
    const arr = filteredItems.map((v, i) => ({ v, i })); // decorate for stability
  ...
  const fetchExportItems = async () => {
    ...
    if (!res.ok) throw new Error("Failed to fetch all ICT equipment report rows.");
  ...
  const exportToExcel = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "ICT Equipment");
      ...
      XLSX.writeFile(wb, `ICT_Equipment_Report_${stamp}.xlsx`);
    } catch (err) {
      console.error(err);
  ...
  const exportGroupSummary = () => {
    ...
    XLSX.writeFile(wb, `ICT_Equipment_Summary_${stamp}.xlsx`);
  };
  ...
  const IconStock = ({ active }) => (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
      <rect
        ...
        strokeWidth="2"
      />
      <path d="M7 7h10v10H7z" fill={active ? "currentColor" : "transparent"} opacity={active ? 0.2 : 0} />
    </svg>
  );
    ...
    return (
      <div className="min-h-[calc(100vh-56px)] flex items-center justify-center bg-gradient-to-b from-white to-gray-50">
        <div role="alert" className="px-4 py-3 rounded-lg bg-red-50 border border-red-100 text-sm text-red-700">
          {error.message || "Failed to load ICT equipment report."}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[calc(100vh-56px)] bg-gradient-to-b from-white to-gray-50">
      <section className="w-full px-4 sm:px-6 lg:px-8 pt-6 pb-10">
        ...
        <div className="flex flex-col gap-3 mb-6">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Filter, group, and export ICT asset reports.
            </p>
          </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row">
                          <SearchInput
                            ...
                            ariaLabel="Search ICT equipment reports"
                            className="w-full"
                  ...
                  <div className="flex flex-wrap items-center gap-2 ml-auto">
                    <button
                      ...
                      className="table-button-accent bg-emerald-500 hover:opacity-95 focus:outline-none focus:ring-2 focus:ring-emerald-300"
        ...
        {/* TABLE CARD */}
        <div className="table-shell table-compact table-flush px-3 md:px-6 pt-3 md:pt-5 pb-4">
                ...
                <table className="w-full whitespace-nowrap hidden lg:table text-left">
                  ...
                  <tbody>
                    {currentItems.length === 0 ? (
                      ...
                      currentItems.map((row) => (
                        <tr
                          key={row?._id || `${row?.property_no}-${row?.itemName}`}
                          className="hover:bg-gray-50 transition-colors duration-150"
                        >
                          {visibleColumnDefs.map((col) => (
                            <td
                              key={`${row?._id || row?.property_no}-${col.key}`}
