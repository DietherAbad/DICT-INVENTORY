// src/components/Reportstableofficeequipments.jsx
...
function compareValues(a, b) {
  const at = typeof a;
  const bt = typeof b;
  // Convert to comparable primitives
  const av =
    a == null
      ? ""
      : at === "number"
...
/** Resolve a user id or name-like into a display name via map (fallback to raw). */
function resolveIdOrName(candidate, userMap) {
  if (!candidate) return "";
  if (userMap && userMap[candidate]) return userMap[candidate];
  // Hide raw ids if they look like ObjectIds
...
/* ---------- Tiny SVG icons (inline, no external deps) ---------- */
const IconSearch = () => (
...
function Reportstableofficeequipments({
  ...
  const STORAGE_KEY = `r02.officeEquipmentReports${storageSuffix}.v5`;
  const STORAGE_COLUMNS_KEY = `r02.officeEquipmentReports${storageSuffix}.columns.v2`;
  ...
  const fetchExportItems = async () => {
    ...
    if (!res.ok) throw new Error("Failed to fetch all equipment report rows.");
  ...
  const exportToExcel = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Office Equipment");
      ...
      XLSX.writeFile(wb, `Office_Equipment_Report_${stamp}.xlsx`);
    } catch (err) {
      console.error(err);
  ...
  const exportGroupSummary = () => {
    ...
    XLSX.writeFile(wb, `Office_Equipment_Summary_${stamp}.xlsx`);
  };

  /* ---------- Stats (enhanced chips) ---------- */
  ...
  // Chip button with icon and active/disabled visual states
  const Chip = ({
    label,
    count,
    colorClasses,
  ...
  // Small inline SVG icons for chips
  ...
  if (error) {
    return (
      <div className="min-h-[calc(100vh-56px)] flex items-center justify-center bg-gradient-to-b from-white to-gray-50">
        ...
        >
          {error.message || "Failed to load office equipment reports."}
        </div>
      </div>
    );
  }
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row">
                          <SearchInput
                            ...
                            placeholder="Search property no, item, classification, project, status, or location"
                            ariaLabel="Search office equipment reports"
                            className="w-full"
                        ...
                        <p className="mt-2 text-[11px] text-slate-500">
                          Searches across property number, item description, classification,
                          asset type, project, status, and current location. The table updates
                          only when you apply the search.
                        </p>
                      </div>
