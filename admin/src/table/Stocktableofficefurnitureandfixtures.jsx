// src/components/Stocktableofficefurnitureandfixtures.jsx
...
/* ---------- Status Pill (now includes disposal states) ---------- */
...
function ItemCard({ data, canManageExtras }) {
  ...
  const showTransfer =
    canManageExtras &&
    (normalizedStatus === "transferred" || normalizedStatus === "for transfer");
  const showDisposal =
    canManageExtras &&
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Link
          to={`/checkitemfurniture/${data._id}`}
...
/* ---------- Row component (now also shows disposal actions) ---------- */
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors duration-150">
      ...
      <td className="px-4 py-3 whitespace-nowrap text-[10px] md:text-xs font-medium text-left border-b relative">
        ...
        {menuOpen && (
          <div
            className="absolute right-0 mt-2 w-56 rounded-xl border border-gray-200 bg-white shadow-xl z-20 overflow-hidden"
          >
            {/* View Item - always when menu is open */}
            <Link
              to={`/checkitemfurniture/${data._id}`}
            ...
            {/* View ITR - Transferred / For Transfer */}
            {(normalizedStatus === "transferred" ||
              normalizedStatus === "for transfer") &&
              canManageExtras && (
              <Link
            ...
            {/* View Disposal for For Disposal / Disposed */}
            {(normalizedStatus === "for disposal" ||
              normalizedStatus === "disposed") &&
              canManageExtras && (
              <Link
            ...
            {/* Info line for Issued (no ITR yet, disposal inside View Item) */}
            ...
            {/* Info line for In Stock / Instock */}
            {(normalizedStatus === "in stock" ||
              normalizedStatus === "instock") && (
              <div className="px-4 py-3 text-[10px] md:text-[11px] text-slate-500 leading-relaxed bg-slate-50 border-t border-gray-100">
                {canManageExtras
...
/* ---------- Main component ---------- */
function Stocktableofficefurnitureandfixtures() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            "Failed to fetch office furniture & fixtures inventory."
          );
        }

        const data = await res.json();
        if (Array.isArray(data)) {
          setInventory(data.reverse()); // newest first
    ...
    return (
      <div className="min-h-[calc(100vh-56px)] flex items-center justify-center bg-gradient-to-b from-white to-gray-50">
        <div className="px-4 py-3 rounded-lg bg-red-50 border border-red-100 text-sm text-red-700">
          {error.message ||
            "Failed to load office furniture & fixtures inventory."}
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-3 mb-6">
          <div>
            <h1 className="text-3xl sm:text-4xl font-light text-slate-900">
              Furniture <span className="font-semibold">&amp; Fixtures</span>
            </h1>
            <p className="mt-2 text-sm text-slate-500">
              Track furniture inventory, issuance, and status updates.
            </p>
          </div>
            ...
            <div className="filter-card-content">
              <div className="table-filters flex-col sm:flex-row sm:items-center xl:hidden">
                ...
                <div className="text-[11px] text-slate-500 sm:text-xs">
                  Showing <span className="font-semibold text-slate-900">{pageItems.length}</span> of{" "}
...
export default Stocktableofficefurnitureandfixtures;
