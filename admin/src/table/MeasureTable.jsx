 ...
 * - use bg-opacity-* and ring-opacity-* instead
 */
...
function MeasureTable() {
  ...
  // keep only one row menu open (cleaner UI)
  const [openMenuId, setOpenMenuId] = useState(null);

  const navigate = useNavigate();

  // ✅ Breadcrumbs: Settings > Unit of Measure
  const breadcrumbs = useMemo(
    () => [
      { label: "Settings", onClick: () => navigate("/settingsdashboard") },
      { label: "Unit of Measure", current: true },
    ],
    [navigate]
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`${BASE_URL}/measure?${params.toString()}`, {
          signal: ac.signal,
        });
  ...
  const submitAdd = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setAddErrorMessage(
            msg || "This unit of measure already exists for the selected designation."
          );
          setAddErrorOpen(true);
          return;
        }
        throw new Error(msg || "Failed to add measure");
      }

      await res.json();
      setAddOpen(false);
      resetAddForm();
      showToast("Unit & measure added successfully.", "success");

      // keep UX simple: go back to page 1 so the user always sees something predictable after add
      setCurrentPage(1);
      setRefreshKey((v) => v + 1);
    } catch (e) {
      showToast(e.message || "Failed to add measure", "error");
    }
  };
  ...
  const onToggleActive = async (item) => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/measure/${item._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 pt-6 pb-10">
        ...
        <div className="flex flex-col gap-3 mb-6">
          <div>
            <h1 className="text-3xl sm:text-4xl font-light text-slate-900">
              Units <span className="font-semibold">of Measure</span>
            </h1>
            <p className="mt-2 text-sm text-slate-500">
              Maintain units/measures across designations. Sort, filter, activate, or deactivate.
            </p>
          </div>
        ...
        <div className="table-shell table-compact table-flush px-4 sm:px-6 py-5">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <div className="text-sm font-semibold text-slate-900">Measures</div>
                  ...
                  <tr>
                    <td colSpan={4} className="px-4 py-10">
                      <EmptyState
                        ...
                        subtitle="We could not find any units that match your filters."
                        hint="Try adjusting filters or add a new unit/measure."
                        className="max-w-2xl"
                      />
                    </td>
                  </tr>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g., box, ream, unit, piece"
              />
        ...
        >
          <div className="space-y-3">
            <p className="text-sm text-gray-700">
              {addErrorMessage || "This unit of measure already exists."}
            </p>
            <div className="flex justify-end gap-2 pt-2">
              <button
