// components/Checkitemictequip.jsx
...
/**
 * Checkitemictequip (aligned with Checkitem master logic)
 *
 * Model fields used:
 ...
 */
export default function Checkitemictequip() {
  const { id } = useParams();
  ...
  const employeeLookupReturn =
    ...
    String(location.state?.from || "").startsWith("/employee-asset-lookup")
      ? location.state.from
      : "";
  const backTarget = employeeLookupReturn || "/stocktableofficeictequipments";
    ...
    (async () => {
      try {
        ...
        const invUrl = isPublicView
          ? `${BASE_URL}/inventoryofficeICTequipment/public/${id}`
          : `${BASE_URL}/inventoryofficeICTequipment/inventory/${id}`;
  ...
  // Terminal / locked statuses: no Issue/Transfer/Disposal requests should be possible.
  const isTerminalStatus = useMemo(() => {
    const terminal = new Set([
      ...
      "decommissioned",
      "retired",
      "for disposal", // already in disposal workflow (still non-actionable)
    ]);
...
<body>
  ...
  const handlePrintHistory = () => {
    ...
    const logoSrc = resolveLogoSrc(DICT);
...
<head>
  ...
  <title>ICT Equipment History - ${escapeHtml(
    inventory.property_no || inventory.itemName || "Item"
  )}</title>
...
<body>
    ...
    <div class="header">
      <div class="brand">
        ...
        <div class="title">
          ...
          <div class="sub">Inventory System — ICT Equipment History Report</div>
        </div>
      </div>
  ...
  const handleDownload = async () => {
    ...
    const hideEls = Array.from(
      wrapper.querySelectorAll('[data-export-hide="true"]')
    );
  ...
  const handleReturnToInventoryConfirm = async (reasonText) => {
    try {
      ...
      const res = await fetch(`${BASE_URL}/inventoryofficeICTequipment/inventory/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(updatedData),
      });
          ...
          <Breadcrumbs
            ...
            items={[
              ...
              employeeLookupReturn
                ...
                : { label: "Office ICT Equipment", to: "/stocktableofficeictequipments" },
              { label: "Item Details" },
            ]}
          />
        )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
            ...
            <div className="p-6 lg:p-7 border-b lg:border-b-0 lg:border-r border-gray-100">
              <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight text-gray-900">
                {inventory.classification || "ICT Equipment"}
              </h2>
          ...
          <div className="px-6 sm:px-8 py-4 border-t border-gray-100 bg-gray-50">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <p className="text-[11px] text-gray-500">
                This ICT equipment is government property. Unauthorized transfer/disposal is prohibited.
              </p>
